...
1. **脚本行为层**：对 `evals/fixtures/` 的合成夹具运行 `scripts/fetch_pubmed.py` 与
   ...
   （c3 去重、c5 渲染）显式双跑验证 sha256。单元测试 29 项：`python3 -m unittest discover tests`。
...
- **第二次联网回归（同日，v0.1.1，11 次请求）**：沿用首次运行 seen 复跑，17 篇全部命中
  去重、0 新增、不再调 efetch；reldate=1/7/30 命中数 11/33/147 单调递增，窗口生效；
  顺带加固渲染分组排序与 seen 重复收敛（单元测试增至 **29 项**）。逐字记录与结论见
  [live_transcript_v2.txt](runs_v1/live_transcript_v2.txt)、
  [live_regression2_2026-09-19.json](runs_v1/live_regression2_2026-09-19.json)、
  [hashes_v2.json](runs_v1/hashes_v2.json)。
